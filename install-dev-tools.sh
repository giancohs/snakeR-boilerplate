...
# Quarto, Chrome, and ChromeDriver. Uses DEVCONTAINER_IMAGE from devcontainer
# when set. Override output dir with: VERSIONS_DIR=/path/to/dir
...
DEVCONTAINER_IMAGE="${DEVCONTAINER_IMAGE:-unknown (set containerEnv.DEVCONTAINER_IMAGE in devcontainer.json to document)}"
...
Set \`containerEnv.DEVCONTAINER_IMAGE\` in \`.devcontainer/devcontainer.json\` to the same value as \`image\` so this line stays accurate.
